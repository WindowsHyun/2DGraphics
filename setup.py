...
mygame = Target(
    script="game_start.py", # 실행되는 py 파일
    dest_base = "CrageneRabbit", #파일의 이름
    icon_resources=[(1, r"CrageneRabbit.ico")], # 아이콘 이름.
...
resources = "".split()   # 게임에서 필요한 리소스를 알려줘야한다. 모든 리소스를 다 입력해야한다.
            # 리소스 폴더를 따로 안적어주고 그냥 옮겨도 상관없다.
